 ...
 *
 * - PaillierMulPlaintext(BIGNUM* result, BIGNUM* ciphertext, BIGNUM* plaintext, BIGNUM* n, BN_CTX* ctx):
 *   This function implements a variant of the homomorphic property of the Paillier cryptosystem,
 *   scalar multiplication of a ciphertext with a plaintext. It returns the result of this operation.
 *
 ...
 *   scalar multiplication in the Paillier cryptosystem. The plaintexts are hardcoded as 5 and 6.
 *
 ...
 * 
 * Compile with:
 * - gcc -o paillier_mul paillier_mul.c -lcrypto
 * 
...
void PaillierMulPlaintext(BIGNUM* result, BIGNUM* ciphertext, BIGNUM* plaintext, BIGNUM* n, BN_CTX* ctx);
// Print utility
...
int main() {
  ...
  // Multiplies the ciphertexts
  BIGNUM* mul_ciphertext = BN_new();
  PaillierMulPlaintext(mul_ciphertext, ciphertextA, plaintextB, n, ctx);

  // Decrypt the mul
  BIGNUM* decrypted_mul = BN_new();
  PaillierDecrypt(decrypted_mul, mul_ciphertext, n, lambda, mu, ctx);

  // At this point, decrypted_mul should contain the mul of plaintextA and plaintextB
  print_BN(plaintextA,"plaintextA",0x0);
  print_BN(plaintextB,"plaintextB",0x0);
  print_BN(ciphertextA,"ciphertextA",0x1);
  print_BN(mul_ciphertext,"mul_ciphertext",0x1);
  print_BN(decrypted_mul,"decrypted_mul",0x0);
...
void PaillierMulPlaintext(BIGNUM* result, BIGNUM* ciphertext, BIGNUM* plaintext, BIGNUM* n, BN_CTX* ctx) {
  ...
  // result = ciphertext^plaintext mod n^2
  BN_mod_exp(result, ciphertext, plaintext, n_square, ctx);
