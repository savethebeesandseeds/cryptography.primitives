 ...
 * File: paillier_sub.c
 *
 * A needs to be grather than B
 *    result = A - B
 * 
 * This file provides an implementation of the Paillier cryptosystem, a 
 * probabilistic asymmetric algorithm for public key cryptography, which has the 
 * additional property of being homomorphic. This makes it possible to perform 
 * operations on encrypted data without decrypting it first. This particular 
 * implementation allows the subtraction of two numbers.
 *
 * Functions included:
 *  - PaillierInitialize: Initializes the public (n, g) and private (lambda, mu) keys
 *  - PaillierEncrypt: Encrypts a plaintext number
 *  - PaillierDecrypt: Decrypts a ciphertext number
 *  - PaillierSubtractCiphertexts: Subtracts two encrypted numbers, producing a 
 *    ciphertext that corresponds to the result of the subtraction operation
 *  - print_BN: Utility function to print BIGNUMs in decimal or hexadecimal form
 *
 * The file also includes a main function that demonstrates how to use these 
 * functions to encrypt two numbers, subtract them (while they are still 
 * encrypted), and then decrypt the result.
 *
 * All arithmetic operations are performed using the OpenSSL BIGNUM library, 
 * which provides support for arbitrary-precision arithmetic.
 *
 * Note: The value of PRIME_SIZE at the beginning of the file determines the size 
 * of the random prime numbers generated for the key initialization. Adjust this 
 * value to increase or decrease the security level.
 *
 * The detailed implementation of each function is provided within the function 
 ...
 * 
 * Compile with:
 * - gcc -o paillier_sub paillier_sub.c -lcrypto
 * 
...
// Subtract
void PaillierSubtractCiphertexts(BIGNUM* result, BIGNUM* ciphertextA, BIGNUM* ciphertextB, BIGNUM* n, BN_CTX* ctx);
// Print utility
void print_BN(BIGNUM *bn, const char *label, int hex);
// Main
int main() {
  BN_CTX* ctx = BN_CTX_new();
  BIGNUM* plaintext_minus_one = BN_new();
  BIGNUM* plaintextA = BN_new();
  BIGNUM* plaintextB = BN_new();
  BIGNUM* ciphertextA = BN_new();
  BIGNUM* ciphertextB = BN_new();
  BIGNUM* sub_ciphertext = BN_new();
  BIGNUM* decrypted_sub = BN_new();

  BN_set_word(plaintextA, 6); // Set value A
  BN_set_word(plaintextB, 5); // Set value B
  ...
  // Subtraction
  PaillierSubtractCiphertexts(sub_ciphertext, ciphertextA, ciphertextB, n, ctx);

  // Decrypt the subtraction
  PaillierDecrypt(decrypted_sub, sub_ciphertext, n, lambda, mu, ctx);

  // At this point, decrypted_mul should contain the sum of plaintextA and plaintextB
  ...
  print_BN(decrypted_sub,"decrypted_sub",0x0);
  ...
  BN_free(plaintextB);
  BN_free(ciphertextA);
  BN_free(plaintext_minus_one);
  BN_free(ciphertextB);
  BN_free(sub_ciphertext);
  BN_free(decrypted_sub);
  BN_CTX_free(ctx);
...
void PaillierSubtractCiphertexts(BIGNUM* result, BIGNUM* ciphertextA, BIGNUM* ciphertextB, BIGNUM* n, BN_CTX* ctx) {
  BIGNUM* n_square = BN_new();
  BIGNUM* inv_ciphertextB = BN_new();

  BN_CTX_start(ctx);

  BN_mul(n_square, n, n, ctx); // n^2

  // Compute inverse of ciphertextB mod n^2
  BN_mod_inverse(inv_ciphertextB, ciphertextB, n_square, ctx);

  // result = (ciphertextA * inv_ciphertextB) mod n^2
  BN_mul(result, ciphertextA, inv_ciphertextB, ctx);
  BN_mod(result, result, n_square, ctx);

  BN_free(n_square);
  BN_free(inv_ciphertextB);
  BN_CTX_end(ctx);
}
// Print utility
void print_BN(BIGNUM *bn, const char *label, int hex) {
